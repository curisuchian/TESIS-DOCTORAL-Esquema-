...
\subsubsection{Preprocesamiento de imagen} \label{Metodología}
...
Para establecer un índice de calidad (QI) del desempeño de la selección automática, se proponen tres índices: el primero ($QI_1$) se obtiene del cociente entre la suma de todos los píxeles de la matriz binaria, obtenida como consecuencia de la intersección de la máscara manual y la máscara automática, y la suma de todos los elementos de la máscara manual. 
In order to establish a quality index (QI) of the performance of the automatic shadow selection, we propose three QIs: the first of them, QI1, is obtained by dividing the sum of all the pixels of the binary matrix obtained by the intersection of both the manual and automatic masks over the sum of the elements of the manual mask (eq. (2)); the second index, QI2, is obtained by dividing the sum of the intersection over the sum of the automatic mask (eq. (3)); and the third index, QI3, is the division of the sum of the intersection over the sum of the union of both the manual and the automatic masks (eq. (4)).
\\
\\
 \begin{equation}
    QI_1=\frac{\Sigma _i_,_j(M_M\cap M_A )}{\Sigma _i_,_j(M_M ) }
    \label{qi1}
\end{equation}
\\
\\
 \begin{equation}
    QI_2=\frac{\Sigma _i_,_j(M_M\cap M_A )}{\Sigma _i_,_j(M_A ) }
    \label{qi2}
\end{equation}
\\
\\
\begin{equation}
    QI_3=\frac{\Sigma _i_,_j(M_M\cap M_A )}{\Sigma _i_,_j(M_M \cup M_A ) }
    \label{qi3}
\end{equation}
\\
\\
where MM is the manual binary mask and MA is the automatic binary mask. Each of the preceding indices takes values from 0 to 1, where 0 indicates not coincident at all and 1 indicates absolute coincidence. In all three indices, the numerator is the intersection between the manual and the automatic masks, because it is a good indicator of coincidence between both masks, while the denominator gives a reference value to parameterize the index. Intersection and union are logical operations applied to every single pixel; thus, it is a condition that both the automatic and manual selection masks that are being compared should be of the same size and of coincident forest areas. At pixel level, intersection involves a logical “and” operation; then, if the value of any pixel belonging either to the manual or to the automatic binary mask is 0, then the result of the operation for that pixel will also be 0. On the other hand, union involves a logical “or” operation; then, if the value of any pixel of either the manual or the automatic mask is 1, then the result of the operation for that pixel will also be 1. The aim of the quality index is to describe how far or close to the manual selection of shadows the algorithm is. In this manner, the highest value corresponding to 1 implies an exact automatic selection, while lower values close to 0 correspond to a low match between selections.

\subsubsection{Etapa de filtrado}
In order to remove noise from the image, a median filter was implemented. To evaluate how the filtering stage affects the performance of the automatic selection, three trials with a neighborhood of three by three, six by six and twelve by twelve pixels were carried out with each image. For each case, a set of three quality indices was proposed, displayed in equations (5), (6), (7).
(5)
(6)
(7)
where M NF is the automatic binary mask obtained without filtering, and MWF is the automatic binary mask processed with filtering.
...
Dos ejemplos de máscaras manuales se muestran en figuras \ref{contorno1} y \ref{contorno2}, con trazo rojo bordeando las regiones sombreadas. La figura \ref{} muestra el solapamiento de la máscara manual (en rojo) con la automática (en azul). En la figura \ref{contorno1} el valor de umbral para la máscara binaria se toma del 60º percentil de la distribución de frecuencias, mientras que en la figura \ref{contorno2} el valor de umbral es tomado del 85º percentil.
