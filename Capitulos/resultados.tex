...
By analyzing Fig. 7, where the relationships between the three QIs proposed and an additional index defined as the difference between the maximum value and the minimum value among the three indices as a function of the percentile value used to calculate the
threshold for the automatic mask, large similarities are observed. In all three cases, a minimum is detectable for the curve that corresponds to the range index (max-min), and this minimum takes place in the 85th percentile. Moreover, for the cases that use both combinations, i.e. blue minus red channel and blue minus green channel, the curve QI3
reaches a maximum at the value of the 85th percentile, while this maximum of the curve is not evident in the case of green minus red channel. Therefore, of the four indexes analyzed, the most appropriate is QI3
, which exhibits a more precise optimum percentile value by the minimum of the curve, and is coincident in the three cases of channel combinations.

\paragraph{Influencia del filtro de mediana en la detección automática de sombras}

Fig. 8a, b and 8c expose how the QIs, calculated following equations (5)–(7), are affected by the implementation of the median filter applied to each image. In the case of the 3 x 3 filter, which had a minor effect on the indices, the indices take values larger than
85\%, for all the forest images. The worst case is that for the 12 x 12 pixel filter, where the comparison between the automatic masks
obtained with and without filter results in a coincidence of more than 70\%.
\paragraph{Evaluación humana de máscaras automáticas de sombra}

In this work, the algorithm results are compared to the manual selections of experts. Since there isn't a 100\% certain base of comparison (experts are human persons and therefore still prone to error of commission or omission), there is a limitation of the quality
index that relies on this issue. Criteria for the manual selection of shadows by experts depends on their particular perception of the
image, varying with the context. On the other hand the algorithm is based on the calculation of an index, and the definition of a
threshold value, which was obtained from a series of experiments with images of a specific type. In other contexts with other types of
images the threshold wouldn't be the same, and even the combination of bands to obtain the index would be different.
After generating a set of automatic masks corresponding to each of the six percentiles using the three color invariant indices of
equations (2)–(4) for the 19 selected images, the resulting 342 masks were compared with the manually obtained masks of the 19 images. By superposing both the automatic and the manually generated masks over the original image, the experts evaluated the quality
of their mutual similarities, giving a score of three levels, namely “good”, “regular” and “bad”. The results are shown in Table 1 and
Fig. 9. As it can be appreciated, for the three color invariant indices, a minimum in the “bad” level is observed around the 90th percentile. For the “regular” and “good” levels, however, the minimum is not very clear, a fact that can be attributed to some subjectivity
of the experts.
...
The procedure proposed using the color invariant index can be applied to shadow detection in aerial images of rainforest areas in
an automatic way, without the need for large computational facilities. The results showed that the 85th percentile from the frequency
distribution of the color invariant index obtained by means of equation (1) and the difference between channels as the numerator are
good parameters to calculate the threshold value for binarization and thus obtain the mask that matches the shadow areas. In the two
cases, the B-G and B–R channels as numerators showed the best results to produce automatic masks. A method of comparison was
found to evaluate the performance of the algorithm, where the QI was defined as a quotient between the resulting intersection of both
the manual and the automatic masks, over the resulting union of both. In this manner, if the index approaches a value of 1, it implies a
higher level of coincidence between the manual and the automatic selection. The results of the algorithm are comparable to the selections of experts, as can be appreciated by looking at the masks overlapped over the images. Although this method has some limitations, it can be applied to RGB images, which also have some capture time conditions, and weather cloud conditions. In order to assure the visibility of shadows in the image, capturing of them must occur from 8 a.m. to 11 a.m. and from 2 p.m. to 5 p.m., depending
on the season and the latitude. Another limitation is capturing images with a covered sky, since shadows appear very weak. However,
this method is a suitable option to automate the task of delineation of shadows on aerial images
