...
Con la intención de abarcar los dos bloques temáticos abordados en esta tesis, este capítulo se divide en dos partes. Inicialmente, se muestran los resultados obtenidos en el análisis de diferentes plataformas de relevamiento de imágenes aéreas en diversas áreas de bosque nativo (Bloque 1) y a continuación se presentan los resultados de los diferentes algoritmos de procesamiento (Bloque 2).
...
En las figuras \ref{PS1} a \ref{PS1 superpuesto} se muestra la imagen de la reserva Parque de la Sierra que, de modo similar a la anterior, se procesó con el método de la ventana deslizante. En este caso se utilizó un umbral de 0,75. En términos comparativos resulta evidente que el valor de umbral más adecuado para la binarización es el de 0,45.
%Luego vienen algunas figuras más la 4.8 , hasta la 4.12. con otra region (PS1) nuevamente puede ser una descricion similar.solo que en este caso se uso un umbral de 0.75, mencionar eso en el texto y al final escribir una comparacion encuanto a la calidad y eso confirma lo que se dijo antes de que 0.45 de valor de umbral para la binarizacion es el seleccinado como mejor umbral.
...
Para el procesamiento morfológico de imágenes se ha utilizado la herramienta R, mostrado características que merecen ser tenidas en cuenta en el análisis. Por lo general esta herramienta es poderosa para el análisis estadístico de datos, y especialmente útil en datos georreferenciados, pero también muy demandante en términos de recursos computacionales, ya que al procesar los datos en archivos raster, la exigencia de memoria RAM es elevada. Asimismo hay rutinas del proceso analizado que demandaron una considerable cantidad de tiempo, incluso varios minutos, como es el caso del comando  normalmixEM() del paquete mixtools (ver \cite{young__aut_mixtools_2022}).
...
\subsubsection{Influencia del valor de percentil y del método de cálculo del índice invariante de color}
...
\begin{figure}[h!]
     \centering
        \includegraphics[width=\textwidth]{Imagenes/qibr.png}
         \caption{Índice de calidad \textpsi \textsubscript{BR}}
         \label{psiBR}
     \end{figure}
     \hfill
     \begin{figure}[h!]
         ...
         \caption{Índice de calidad \textpsi \textsubscript{BG}}
         \label{psiBG}
     \end{figure}
     \hfill
     \begin{figure}[h!]
         ...
         \caption{Índice de calidad de \textpsi \textsubscript{GR}}
         \label{psiGR}
     \end{figure}
...
En el análisis de la figura \ref{quality_index}, se observan grandes similitudes, donde se grafican las relaciones entre los tres índices de calidad propuestos y un índice adicional definido como la diferencia entre el valor máximo y el mínimo entre los tres índices en función del valor de percentil usado para calcular el umbral de la máscara binaria automática. En los tres casos se detecta un valor mínimo para la curva que corresponde al índice adicional, es decir el rango (max - min), y este valor mínimo corresponde al percentil 85º. Además para los casos que usan sendas combinaciones en la ecuación \ref{invariante de color} resta de canal azul menos rojo y azul menos verde, la curva del índice $QI_3$ alcanza un valor máximo en el que corresponde al 85º percentil, mientras que este punto máximo no se hace evidente para el caso de la combinación verde menos rojo. Por lo tanto de los cuatro índices que fueron analizados, el más conveniente es el $QI_3$, que exhibe un valor óptimo de percentil más preciso por medio del mínimo en la curva, y en los tres casos de combinaciones de canales es coincidente.
Así se considera entonces que el índice de calidad QI3 (identificado como `int/union` en los gráficos) se considera el más determinante porque busca un balance entre maximizar la coincidencia (intersección) y controlar el tamaño de la máscara para evitar la selección de áreas incorrectas (unión).  El análisis demuestra que este índice alcanza su valor máximo precisamente cuando se utiliza el percentil 85 como umbral. Un QI3 máximo indica el mejor acuerdo posible entre la detección automática y la selección manual de referencia.
También, según se observa al analizar el índice que mide la diferencia entre el valor máximo y el mínimo de los tres índices de calidad (`max-min` en el gráfico rojo de la Figura 4.40), este indice alcanza su valor mínimo en el percentil 85. Un rango mínimo sugiere que en ese punto los diferentes índices de evaluación son más consistentes entre sí, lo que refuerza al percentil 85 como el valor de umbral más estable y fiable.
Así, el percentil 85 es el valor óptimo porque simultáneamente maximiza el índice de calidad más relevante (QI3) y minimiza la discrepancia entre las distintas métricas de evaluación, demostrando ser el umbral más robusto y preciso en este análisis.

\subsubsection{Influencia del filtro de mediana en la detección automática de sombras}

Las figuras \ref{filter3x3}, \ref{filter6x6} y \ref{filter12x12} ilustran el efecto de aplicar un filtro de mediana con ventanas de tamaño creciente (3x3, 6x6 y 12x12, respectivamente) sobre los índices de calidad obtenidos. La aplicación de un filtro de 3x3 produce una alteración despreciable en los resultados, ya que los índices de calidad se mantienen consistentemente por encima del 85\% para todas las imágenes. Por el contrario, el uso de una ventana de 12x12 demuestra ser contraproducente, pues genera la mayor degradación en el rendimiento y reduce la métrica de coincidencia a poco más del 70\%

\subsubsection{Evaluación humana de máscaras automáticas de sombra}

En este trabajo los resultados del algoritmo se comparan con la selección manual llevada a cabo por expertos. Partiendo de que no hay una base de certeza absoluta, existe una limitación en el índice de calidad que se basa en este aspecto. El criterio para la selección manual de parte de los expertos depende de su particular percepción de la imagen, variando según el contexto. Por otro lado el algoritmo se basa en el cálculo de un índice y la definición de un valor umbral. En otros contextos con otro tipo de imágenes el valor de umbral no sería el mismo, incluso la combinación de bandas usadas en la ecuación \ref{invariante de color} para obtener el índice sería diferente.
...
\subsubsection{Validación del algoritmo IIC por umbral y por combinaciones de canales de color.} %esto va en capítulo IIC
\paragraph{Validación del algoritmo IIC con umbral fijo (percentil 85°) en diferentes combinaciones de canales de color.}
A partir de los resultados obtenidos y que se muestran en \ref{Resultados IIC}, se realizaron pruebas del algoritmo sobre distintas imágenes que contienen sombra, con luz natural y con luz artificial, para ver su desempeño en la selección automática de sombras. En todos los casos el umbral de binarización fue definido considerando el percentil 85° de la distribución de frecuencias del índice invariante de color calculado por la ecuación \ref{invariante de color 1}:
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ECUACIÓN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\
\begin{equation}
	\psi\textsubscript{B1B2}=\frac{4}{\pi} arctan\left(\frac{B\textsubscript{1}-B\textsubscript{2}}{B\textsubscript{1}+B\textsubscript{2}}\right),\label{invariante de color 1}
\end{equation}
\\
donde $B_1$ y $B_2$ son dos bandas diferentes de color, correspondientes al espacio de color RGB.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\
Las imágenes \ref{info1-1}, \ref{info1-3}, \ref{info1-5} y \ref{info1-7} se procesaron con la combinación de las bandas azul y verde ($\Psi_{BG}$), mientras que las imágenes \ref{info1-2}, \ref{info1-4}, \ref{info1-6} y \ref{info1-8} con la combinación azul y rojo ($\Psi_{BR}$). No se incluye el análisis con la combinación de bandas verde y roja porque el resultado de usar esa combinación diverge notablemente respecto a las otras dos combinaciones utilizadas y no se corresponde con las áreas de sombra en las imágenes procesadas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1izquierda1.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1derecha1.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1izqquierda2.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-3}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1derecha2.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-4}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1izquierda3.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-5}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1derecha3.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-6}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1izquierda4.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-7}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info1derecha4.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info1-8}
\end{figure}

Se observa que hay mucha similitud en las áreas marcadas, no obstante los diferentes valores de binarización, obtenidos por criterio del percentil 85° de la distribución de frecuencias de los valores de índice invariante de color hallados para cada combinación de bandas.
\\
\\
\paragraph{Determinación de Umbral de Binarización por Histograma de Frecuencias
}
Aplicando el algoritmo sobre una imagen seleccionada \ref{info2_flor} que contiene sombras, usando la ecuación \ref{invariante de color 1} en la combinación de bandas verde y roja, se obtiene una distribución de los valores del índice invariante de color, representado en el histograma de la figura \ref{info2-hist}. En el mismo se observa una distribución bimodal, y el valle entre ambos picos se sitúa en torno al valor 0,2. Seleccionando este valor como umbral de binarización se obtiene la máscara de sombra representada en la Fig. \ref{info2-mascara}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info2_flor.jpg}
    \caption{Imagen con presencia de sombras }
    \label{info2-flor}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info2.jpg}
    \caption{Imagen con máscara de sombras (contorno azul)}
    \label{info2-mascara}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes/Informes/info2_histograma.jpg}
    \caption{Histograma de frecuencias del índice invariante de color }
    \label{info2-hist}
\end{figure}

Observando la imagen con la máscara superpuesta en color azul se nota una marcación altamente coincidente con la que corresponde al área sombreada.

\paragraph{Evaluación de la Eficacia de Combinaciones de Bandas para la Selección de Sombras}

De la ecuación \ref{invariante de color 1} se desprende que para una imagen compuesta por tres bandas de color, rojo, verde y azul, resultan en seis diferentes índices invariantes de color para una imagen dada, lo cual se representa en la figura \ref{hexagono}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Comparativo Homo-IIC/hexagono.JPG}
    \caption{Hexágono de posibles combinaciones del índice invariante de color (ψ) a partir de dos de las tres bandas de color (Rojo, Verde y Azul).}
    \label{hexagono}
\end{figure}

En la figura \ref{hexagono1} se observan los resultados de las seis combinaciones diferentes del cálculo de índice invariante de color, aplicados a una imagen que presenta tres niveles homogéneos de colores azul, verde y rojo, dentro de un entorno natural (figura \ref{sanjose}).
...
Según se aprecia en la figura \ref{hexagono1} comparando los resultados del cálculo del índice invariante de color para las seis combinaciones contra la imagen original (\ref{sanjose}) hay mayor nitidez de sombras para las combinaciones rojo menos verde, rojo menos azul y azul menos verde. 

Para el caso de la imagen de la figura \ref{costanera} se seleccionó una imagen con un área grande de color rojo, en ese caso la distinción de sombras es más nítida en las combinaciones de resta de canal verde menos canal rojo y canal rojo menos canal azul (ver figura \ref{hexagono2}.
...
%En el caso de aplicar las seis combinaciones posibles del índice invariante de color a la imagen de la figura \ref{planta}, la distinción de sombras es más evidente para la combinación de la resta entre canal verde menos canal rojo (ver figura \ref{hexagono3})


Aplicando las combinaciones del índice invariante de color a la imagen de la figura \ref{colon}, figura que contiene grandes áreas de colores rojo y verde principalmente, y en entorno natural, resulta más notoria la distinción de sombras para la combinación de la resta de canal verde menos canal azul (ver figura \ref{hexagono4}) 
...
\paragraph{Influencia de la Resolución de la Imagen en la Detección de Sombras}
 Con el objetivo de determinar la incidencia del tamaño o resolución de la imagen en píxeles en el desempeño del algoritmo del índice invariante de color, se probaron diferentes tamaños de imagen, de 500 x 281 píxeles, 1000 x 562 píxeles, 2000 x 1125 píxeles y 8000 x 4500 píxeles. A simple vista puede corroborarse que la incidencia es ínfima, las máscaras resultantes de la selección de sombra son muy similares (ver Figuras \ref{500x261} a \ref{8000x4500-2}), como es esperable, ya que el índice invariante de color opera sobre cada pixel de manera individual, sin incorporar efectos de vecindades o grupos de píxeles.
 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-1.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 500 x 281 píxeles}
    \label{500x261}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-1.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 1000 x 562 píxeles}
    \label{1000x562}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-1.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 2000 x 1125 píxeles}
    \label{2000x1125}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-1.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 8000 x 4500 píxeles}
    \label{8000x4500}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-5.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 500 x 281 píxeles}
    \label{500x261-2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-6.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 1000 x 562 píxeles}
    \label{1000x562-2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-7.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 2000 x 1125 píxeles}
    \label{2000x1125-2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info4-8.png}
    \caption{Máscara de sombras seleccionadas de imagen con resolución de 8000 x 4500 píxeles}
    \label{8000x4500-2}
\end{figure}

\paragraph{Implementación de algoritmo en diferentes plataformas}
Se probó implementar el algoritmo en diferentes plataformas, tanto en Matlab y Octave que comparten las líneas de código en los scripts, y también se adaptó para ser ejecutado el código en la plataforma R. Asimismo se hizo una prueba usando una planilla de cálculo, usando la fórmula para calcular el índice invariante de color y por último se usó el programa QGIS para procesamiento de datos georreferenciados y que permite realizar el cálculo entre capas ráster. En todos ellos se pudo obtener a la salida una máscara binaria que corresponde con las áreas sombreadas de la imagen. En el caso de la prueba realizada con la planilla de cálculo se observa que resulta una tarea muy pesada para el programa, ya que este no está optimizado para procesar imágenes. Por tal motivo a los efectos de comparar, se usó un tamaño de imagen reducido, de 400 x 225 píxeles para todos los casos. La imagen utilizada para procesar es mostrada en la Figura \ref{info5-original}. Las Figuras \ref{info5-matlab} a \ref{info5-r} muestran los resultados de las máscaras de sombra obtenidas en cada plataforma.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info5-1.png}
    \caption{Imagen a ser procesada con el algoritmo de detección de sombras en diferentes plataformas}
    \label{info5-original}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info5-matlab.png}
    \caption{Máscara de sombras de la imagen \ref{info5-original} implementando el algoritmo en Matlab}
    \label{info5-matlab}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info5-octave.png}
    \caption{Máscara de sombras de la imagen \ref{info5-original} implementando el algoritmo en Octave}
    \label{info5-octave}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info5-qgis.png}
    \caption{Máscara de sombras de la imagen \ref{info5-original} implementando el algoritmo en QGIS}
    \label{info5-QGIS}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info5-excel.png}
    \caption{Máscara de sombras de la imagen \ref{info5-original} implementando el algoritmo en una planilla de cálculo}
    \label{info5-excel}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes//Informes/info5-r.png}
    \caption{Máscara de sombras de la imagen \ref{info5-original} implementando el algoritmo en R}
    \label{info5-r}
\end{figure}
A simple vista puede apreciarse que las máscaras binarias resultantes son muy similares. Las que corresponden al procesamiento con Matlab y con Octave (figuras \ref{info5-matlab} y \ref{info5-octave} respectivamente) son idénticas entre sí porque usan el mismo script. En el caso del procesamiento  El tamaño de archivos que se generan con el procesamiento en hojas de cálculo hace que sea una opción menos atractiva para este tipo de tareas. Pero queda visto que igualmente puede implementarse.

Se evaluó el tiempo que lleva la ejecución del algoritmo, probando en diferentes plataformas, Matlab, Octave, R. Las pruebas se hicieron con imágenes en distintas resoluciones, de 500 x 281, 1000 x 562, 2000 x 1125, 4000 x 2250 y 8000 x 4500 píxeles. Las pruebas que se corrieron en las plataformas Matlab y Octave no difirieron demasiado en el tiempo, que prácticamente ha sido similar en todos los casos, con un tiempo de alrededor de 15 segundos para la imagen de 8000 x 4500 px en Matlab, y 22 segundos en Octave. En cambio fue notorio el incremento de tiempo de ejecución para el caso en R, para la imagen de 4000 x 2250 píxeles llevó alrededor de 17 segundos, y la de 8000 x 4500 píxeles fue imposible determinar el tiempo, ya que desbordó la memoria RAM utilizada, en este caso de 8 GB. La proporción del incremento del tiempo de ejecución en relación al tamaño de la imagen es de alrededor de tres veces por cada cuadruplicación en resolución de imagen.

\paragraph{Análisis del efecto de la hora del día en la detección de sombras}
La presencia de sombras en imágenes aéreas también depende del momento en que se realiza la captura de laimagen, por esa razón se evaluó para una misma escena capturada en dos momentos diferentes del día, antes del mediodía y después del mediodía, para comprobar el desempeño del algoritmo ante la variación de esas condiciones. Las imágenes fueron procesadas con el algoritmo del método del índice invariante de color, usando distintos valores de percentil para seleccionar el umbral, 60°, 70°, 80°, 85°, 90° y 95°. Las figuras \ref{info7-4pm60} a \ref{info7-8am90} muestran los resultados.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-4pm60.jpg}
	\caption{Imagen capturada después del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 60.}
	\label{info7-4pm60}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-4pm70.jpg}
	\caption{Imagen capturada después del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 70.}
	\label{info7-4pm70}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-4pm80.jpg}
	\caption{Imagen capturada después del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 80.}
	\label{info7-4pm80}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-4pm85.jpg}
	\caption{Imagen capturada después del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 85.}
	\label{info7-4pm85}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-4pm90.jpg}
	\caption{Imagen capturada después del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 90.}
	\label{info7-4pm90}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-4pm95.jpg}
	\caption{Imagen capturada después del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 95.}
	\label{info7-4pm95}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-8am80.jpg}
	\caption{Imagen capturada antes del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 80.}
	\label{info7-8am80}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-8am85.jpg}
	\caption{Imagen capturada antes del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 85.}
	\label{info7-8am85}
\end{figure}
\begin{figure}[h]
	\centering
	\includegraphics[width=\linewidth]{Imagenes/Informes/info7-8am90.jpg}
	\caption{Imagen capturada antes del mediodía con máscara de sombras procesada con el algoritmo del índice invariante de color usando un umbral del percentil 90.}
	\label{info7-8am90}
\end{figure}

\subsection{Comparación entre algoritmos de detección de sombras (procesamiento homomórfico y el índice invariante de color)}
Las figuras \ref{180_fzz}, \ref{180_eq_fzz}, \ref{180_iic} y \ref{180_eq_iic} muestran la misma escena forestal, pero capturada bajo condiciones diferentes: una imagen está sin ecualizar y la otra ha sido ecualizada. El objetivo es evaluar y comparar la robustez de los dos algoritmos de identificación de sombras en las imágenes—el procesamiento homomórfico y el Índice Invariante de Color (IIC)—frente a estas variaciones en las condiciones de la imagen.

\input{Figuras/comparativo_iic_fzz}
Analizando los resultados de ambos procesamientos se encontró que al aplicar el algoritmo homomórfico a las dos imágenes (una ecualizada y la otra sin ecualizar), se observan diferencias significativas en los resultados del procesamiento. Esto indica que el algoritmo es sensible a las alteraciones en la iluminación y el contraste de la imagen. Por otro lado, cuando se ejecuta el algoritmo de IIC en las mismas dos imágenes, los resultados son prácticamente idénticos. Esto confirma que el IIC es altamente insensible a las variaciones de la imagen, lo que lo convierte en un método más fiable para el análisis en condiciones no controladas. Por ello, los resultados demuestran la superioridad del algoritmo de IIC en términos de robustez.